...
分支独有问题使用`CLUSTER-Bxxx`编号，不计入以`master d1aef7ff`为基线的主报告346项统计。
...
- 共享net被动close registry候选经GC反查排除：TCP/TLS/UDP handle的`__gc = conn.close`会清表且registry不保活handle；cluster自身`close_fd`也显式清表。
