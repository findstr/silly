...
统计口径为 71 条：5 个 CORE + 5 个 SOCK + 7 个 HTTP/1 + 8 个 WebSocket + 26 个 HTTP/2 + 3 个 HPACK + 17 个 gRPC；以主报告中的编号和证据为准。
...
HTTP/1 framing 首轮已确认 `HTTP1-001` 至 `HTTP1-007`；WebSocket 首轮已确认 `WS-001` 至 `WS-008`；HTTP/2 已确认 `H2-001` 至 `H2-026`，HPACK 已确认 `HPACK-001` 至 `HPACK-003`。gRPC 已确认 `GRPC-001` 至 `GRPC-017`；custom metadata 目前记为未公开支持而非 mandatory 偏离。下一项收尾核对 call lifecycle、文档/API与未编号功能缺口。每次只记录一个规范结论，不新增复现代码。
