...
统计口径为 44 条：5 个 CORE + 5 个 SOCK + 7 个 HTTP/1 + 8 个 WebSocket + 16 个 HTTP/2 + 3 个 HPACK；以主报告中的编号和证据为准。
...
HTTP/1 framing 首轮已确认 `HTTP1-001` 至 `HTTP1-007`；WebSocket 首轮已确认 `WS-001` 至 `WS-008`。HTTP/2 已确认 `H2-001` 至 `H2-016`，HPACK 已确认 `HPACK-001` 至 `HPACK-003`。下一项继续检查 RST/HEADERS stream state。每次只记录一个规范结论，不新增复现代码。
