...
统计口径为 12 条：5 个 CORE + 5 个 SOCK + 2 个 HTTP/1；以主报告中的编号和证据为准。
...
消息体长度优先级已经确认 `HTTP1-001`（TE+CL 请求按 TE 处理后没有强制关闭连接）和 `HTTP1-002`（相同 `Content-Length` 列表被拒绝）。下一项从非 final chunked transfer-coding 和客户端响应 framing 继续；每次只记录一个规范结论，不新增复现代码。
