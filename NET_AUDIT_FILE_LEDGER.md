...
> 状态：静态覆盖已收口；389 个纳入范围的实际文件均有逐路径结论，发布仍由主报告中的未修复问题阻断
...
每个路径单列一行。`问题/证据`只写最直接映射，不表示该文件只与这些问题相关。最终已用仓库实际文件集合做差集：运行时代码、测试、LuaLS、双语文档与VuePress配置均已逐文件列入，差集为空。
...
| `lualib/silly/console.lua` | 已审有归档 | CORE-008/013、METRIC-003 | 观测输出与production hotfix control plane |
...
1. 实际文件宇宙为389项：`src/**` 47项、`luaclib-src/**` 51项、`lualib/**` 87项、`test/**` 65项、`docs/src/**/*.md` 133项及`docs/src/.vuepress/*.ts` 6项；图片、字体、样式等不表达API/协议契约的展示资产按第7节规则排除。
2. 上述实际路径集合与本台账首列的389个唯一反引号路径双向差集为空，无漏项、无虚构路径、无重复路径。
3. 文件状态为`已审有归档`250项、`已审无新增`80项、`范围外`59项；没有`待审`或`审阅中`行，范围外项均在对应行写明排除理由。
4. 静态覆盖收口不表示实现无bug或允许发布；376项master问题与4项cluster分支独有问题仍待修复。独立peer、畸形输入、并发barrier、fault injection、版本矩阵与修复后sanitizer按用户约束进入修复阶段。
