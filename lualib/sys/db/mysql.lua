...
local function _mysql_login(self, user, password, database, on_connect)
	return function(channel)
		...
		local ok, packet = dispatch_resp(channel)
		if not ok then
			error "bad handshake recv packet: bad connection"
		end
		...
		local ok, err = channel:request(authpacket, dispatch_resp)
		if on_connect then
			on_connect(self)
		end
		return ok, err
...
function _M.create(opts)
	...
	local db = opts.database or ""
	local usr = opts.user or ""
	local pwd = opts.password or ""

	self.channel = dispatch:create {
		addr = opts.host,
		auth = _mysql_login(self, usr, pwd, db, opts.on_connect),
	}
	return self
end
...
function _M.query(self, query)
	...
	return channel:request(querypacket, self.query_resp)
end
...
function _M.prepare(self, sql)
	...
	return channel:request(querypacket, self.prepare_resp)
end
...
function _M.execute(self, stmt, ...)
	...
	return channel:request(querypacket, self.execute_resp)
end
...
function _M.stmt_close(self, stmt)
	...
	local channel = self.channel
	return channel:request(querypacket)
end
