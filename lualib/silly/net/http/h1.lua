...
local function read(s, size, timeout)
	...
	if len == chunked then
		while totalsize < size and not s.eof do
	...
	if totalsize and totalsize < size then
		return "", "end of file"
	end
	if err then
...
local function newstream(scheme, conn, addr, ver, method, path, header, readexpect)
...
local function waitresponse(s, timeout)
	...
	if not ver or not status then
		local err = "invalid response line"
		s.err = err
		return false, err
	end
	...
	if not nobody then
		...
		if cl then
			...
			if not n or n < 0 then
				local err = "invalid content-length"
				s.err = err
				return false, err
			end
			s.readexpect = n
...
function M.newstream(scheme, conn, addr, release)
	local s = newstream(scheme, conn, addr, "HTTP/1.1", "", "", {}, 0)
...
---@param addr string
---@param scheme string
function M.httpd(handler, conn, addr, scheme)
	...
	while true do
		...
		local stream = newstream(scheme, conn, addr, ver, method, path, header, len)
