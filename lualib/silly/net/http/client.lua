...
---@field remoteaddr string -- remote address
...
local function find_conn(client, key, scheme)
	...
	for i = #h1entries, 1, -1 do
		...
		if entry.conn:isalive() then
			...
			tremove(h1entries, i)
			return h1.newstream(scheme, entry.conn, entry.remoteaddr, releaseh1)
		end
	end
	return nil
end
...
local function connect(client, scheme, host, port)
	...
	if alpnproto and alpnproto(conn) == "h2" then
		local channel, err = h2.newchannel(scheme, conn, addr)
		if not channel then
			conn:close()
			return nil, err
		end
	...
	return h1.newstream(scheme, conn, addr, releaseh1), nil
end
