...
local function try_create_rr(name, qtype)
	...
	if not rr then
		rr = {name = name, qtype = qtype, ttl = 0, version = 0}
		by_name[qtype] = rr
	end
	return rr
end
...
local function finish_req(req, errmsg)
	print(string.format("[DNS_DBG] finish_req: errmsg=%s rr=%s", tostring(errmsg), tostring(req.rr))) --DEBUG_DNS
...
local function dispatch_resp(msg)
	...
	print(string.format("[DNS_DBG] dispatch_resp: id=%s name=%s qtype=%s tc=%s #records=%s", tostring(id), tostring(name), tostring(qtype), tostring(tc), records and #records or "nil")) --DEBUG_DNS
	if not id then
		return nil
	end
	local by_name = name_cache[name]
	...
	print(string.format("[DNS_DBG] dispatch_resp: rr=%s req=%s version_match=%s", tostring(rr), tostring(req), tostring(rr and rr.version == id))) --DEBUG_DNS
	...
	if #records > 0 then
		...
		for _, rec in ipairs(records) do
			...
			print(string.format("[DNS_DBG]   record: rname=%s rqtype=%s ttl=%s rdata=%s", tostring(rname), tostring(rqtype), tostring(ttl), tostring(rdata))) --DEBUG_DNS
		...
		print(string.format("[DNS_DBG] dispatch_resp: seen[rr]=%s rr.ttl=%s", tostring(seen[rr] ~= nil), tostring(rr.ttl))) --DEBUG_DNS
	end
	local first = rr[1]
	local errmsg = not first and "nxdomain" or nil
	print(string.format("[DNS_DBG] dispatch_resp: first=%s errmsg=%s", tostring(first), tostring(errmsg))) --DEBUG_DNS
...
local function retry_cb(req)
	if req.rr.version ~= req.version then
...
---@param req silly.net.dns.req
---@return boolean
function send_udp_req(req)
	local sent = false
	local pkt = req.pkt
	print(string.format("[DNS_DBG] send_udp_req: #servers=%d rr_name=%s", #servers, req.rr and req.rr.name or "?")) --DEBUG_DNS
	for i, srv in ipairs(servers) do
		local err
		local conn = srv.udp_conn
		print(string.format("[DNS_DBG] send_udp_req: srv[%d] addr=%s has_conn=%s", i, srv.addr, tostring(conn ~= nil))) --DEBUG_DNS
		if not conn then
			conn, err = udp.connect(srv.addr)
			if not conn then
				logger.error("[dns] connect", srv.addr, "failed:", err)
				print(string.format("[DNS_DBG] send_udp_req: srv[%d] connect FAILED: %s", i, tostring(err))) --DEBUG_DNS
			else
				task.fork(udp_recv_loop, srv)
				srv.udp_conn = conn
				print(string.format("[DNS_DBG] send_udp_req: srv[%d] connected OK", i)) --DEBUG_DNS
			end
		end
		if conn then
			local ok = conn:sendto(pkt)
			print(string.format("[DNS_DBG] send_udp_req: srv[%d] sendto=%s", i, tostring(ok))) --DEBUG_DNS
			if ok then
				sent = true
			end
		end
...
local function query(name, qtype, timeout)
	print(string.format("[DNS_DBG] query: enter name=%s qtype=%s", name, qtype)) --DEBUG_DNS
	local ctx
	...
	local ok = wait()
	print(string.format("[DNS_DBG] query: wait returned ok=%s (type=%s)", tostring(ok), type(ok))) --DEBUG_DNS
	if ok == TIMEOUT then
		return false
	end
	time.cancel(timer)
...
local function findcache(name, qtype)
	local now = timenow()
	local original = name
	print(string.format("[DNS_DBG] findcache: enter name=%s qtype=%s now=%s", name, qtype, now)) --DEBUG_DNS
	for i = 1, 100 do
		local rrs = name_cache[name]
		if not rrs then
			print(string.format("[DNS_DBG] findcache: no name_cache for '%s', original='%s'", name, original)) --DEBUG_DNS
		...
		local rr = rrs[qtype]
		print(string.format("[DNS_DBG] findcache: name=%s rr=%s rr.ttl=%s valid=%s", name, tostring(rr), rr and tostring(rr.ttl) or "N/A", tostring(rr and rr.ttl >= now))) --DEBUG_DNS
		if rr and rr.ttl >= now then
			print(string.format("[DNS_DBG] findcache: HIT name=%s rr[1]=%s #rr=%s", name, tostring(rr[1]), #rr)) --DEBUG_DNS
			return rr, nil
		end
		local cname = rrs[RR_CNAME]
		print(string.format("[DNS_DBG] findcache: cname=%s cname.ttl=%s cname[1]=%s", tostring(cname), cname and tostring(cname.ttl) or "N/A", cname and tostring(cname[1]) or "N/A")) --DEBUG_DNS
		if cname and cname.ttl >= now and cname[1] then
			name = cname[1]
			print(string.format("[DNS_DBG] findcache: following CNAME to '%s'", name)) --DEBUG_DNS
		else
			if name ~= original then
				return nil, name
			end
...
local function resolve_r(name, qtype, timeout, deep)
	print(string.format("[DNS_DBG] resolve_r: enter name=%s qtype=%s deep=%s", name, qtype, deep)) --DEBUG_DNS
	if deep > 100 then
		return nil
	end
	local rr, cname = findcache(name, qtype)
	print(string.format("[DNS_DBG] resolve_r: first findcache rr=%s cname=%s", tostring(rr), tostring(cname))) --DEBUG_DNS
	if not rr and not cname then
		query(name, qtype, timeout)
		rr, cname = findcache(name, qtype)
		print(string.format("[DNS_DBG] resolve_r: after query findcache rr=%s cname=%s", tostring(rr), tostring(cname))) --DEBUG_DNS
	end
	if cname then
		print(string.format("[DNS_DBG] resolve_r: following cname=%s", cname)) --DEBUG_DNS
		return resolve_r(cname, qtype, timeout, deep + 1)
	end
	print(string.format("[DNS_DBG] resolve_r: returning rr=%s rr[1]=%s", tostring(rr), rr and tostring(rr[1]) or "N/A")) --DEBUG_DNS
	return rr
end
