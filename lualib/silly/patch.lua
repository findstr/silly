...
local function patch(run, runup, fix, fixup)
	...
	for i = 1, info.nups do
		...
		if fixup[name] then
			...
			assert(rup.idx == i)
			if type(val) == "function" then
				assert(runup[name].val == val)
				patch(rup.val, rup.up, fup.val, fup.up)
			else
...
return function(runfunc, fixfunc, fixenv)
	local runup = {}
	local fixup = {}
	collectup(runfunc, runup)
	collectup(fixfunc, fixup)
	patch(runfunc, runup, fixfunc, fixup)
