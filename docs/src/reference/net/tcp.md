...
- **返回值**:
  ...
  - 失败: `nil, silly.errno` - nil 和传输层错误（见 [silly.errno](../errno.md)）
- **示例**:
...
- **返回值**:
  ...
  - 失败: `nil, silly.errno` - nil 和传输层错误（例如 `opts.timeout` 触发时为 `errno.TIMEDOUT`、`errno.CONNREFUSED`、`errno.RESOLVE` 等）。详见 [silly.errno](../errno.md)。
...
task.fork(function()
    ...
    if not conn2 then
        print("Connect failed:", err2)  -- 超时时 err2 == errno.TIMEDOUT
        return
    end
    conn2:close()
end)
...
- **返回值**:
  - 成功: `true`
  - 失败: `false, silly.errno` - 若套接字已关闭则通常为 `errno.CLOSED`
- **示例**:
...
- **返回值**:
  - 成功: `true`
  - 失败: `false, silly.errno` - 传输层错误（例如 `errno.CLOSED`、`errno.PIPE`）
- **示例**:
...
- **返回值**:
  ...
  - **EOF**: `nil, errno.EOF` - 对端正常半关闭（有序关闭）
