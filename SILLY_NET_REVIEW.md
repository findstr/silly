...
> 状态：1.0 `net` 静态完成性反证审计已收口；归档376项master问题与4项cluster分支独有问题，发布继续阻断
...
### DOC-061 — P2 — logging “生产级”示例异常路径永久抬高 in-flight gauge 并可能二次响应

- 状态：已确认；双语完整示例的异常边界、metrics更新顺序和HTTP response终局的确定性静态核对。本轮不触发handler异常或采集指标。
- 位置：`docs/src/{en/,}guides/logging-monitoring.md:865-1074`的`handle_request`开头执行`http_requests_in_flight:inc()`，只有所有路由、response、四项metrics和结构化日志都成功后才在`:1048-1057`执行`dec()`。外层`silly.pcall` catch在`:1060-1073`只记录错误、发送500并增加500 counter，不递减gauge、观察duration/request/response size或判断response是否已提交。
- 触发：`json.encode`、业务handler、`time.sleep`恢复、`stream:respond/closewrite`、metric label/observe或`log_request`中任一步抛错。`DOC-057`所述非法logger格式属于前一个简化示例；本完整示例本身仍可由业务/codec/transport异常进入该路径。
- 影响：每次异常都让`api_http_requests_in_flight`永久加一，持续故障使表示“当前处理请求”的gauge单调上升，即使所有请求协程已结束。容量告警、自动扩缩容和Grafana并发面板因此报告幽灵负载；异常请求又缺duration/size观测，故障时恰好失去延迟证据。若错误发生在200/201/404响应已经提交后，catch仍发送第二个500，产生协议异常或误导日志，并可能同时记录原status与500两次。
- 证据：Lua/xpcall异常直接跳过`handle_request`余下语句，不存在defer/finally；gauge对象不会按协程生命周期自动回滚。catch没有保存“已响应”状态，HTTP stream也不会撤销先前写入。正常分支的counter/duration/dec位置均早于最终log但晚于全部response，所以不同抛错点还会产生counter已计/gauge已减与完全漏计等多种不一致终局。
- 根因：instrumentation分散在业务handler尾部，把“顺利走到最后”当成唯一终局，没有以scope guard/outer middleware保证in-flight恰好减一次；response提交与exception recovery也没有单一所有者。
...
### DOC-076 — P2 — Echo 教程声称 accept callback 返回会自动关连接，正常返回实际依赖不确定的 GC 才 close

- 状态：已确认；双语callback生命周期说明、TCP accept wrapper、弱引用pool与connection metatable完整初始化顺序的确定性静态核对。本轮不接受连接或触发GC。
- 位置：`docs/src/{en/,}tutorials/echo-server.md:345-349`称callback“returns or encounters an error”时framework自动关闭connection；实际`lualib/silly/net/tcp.lua:103-111`只在`silly.pcall(lc.accept,s)`返回失败时立即调用`s:close()`，成功返回没有收尾。`:51`的`conn_pool`是弱value表；metatable虽在`:55-59`先以nil占位，但`:245-246`随后安装`__gc/__close=conn.close`，因此正常return只能等待对象不可达后的GC finalizer。
- 触发：accept callback在校验失败、限流、协议分支或业务early-return时没有显式`conn:close()`；也包括用户依据教程文字认为正常return足以释放connection。
- 影响：callback返回时底层socket/fd仍然打开，释放时间取决于GC调度而非callback scope；GC暂停/关闭、低分配长周期或大量early-return时会积累fd/socket slot，并在返回后继续接收/缓冲数据。应用已丢失局部owner，无法主动控制精确关闭时刻；教程承诺会让代码review误判ownership已经同步收尾。
- 证据：accept wrapper的唯一同步close语句受`if not ok`保护，正常`ok=true`直接return；weak pool不延长owner生命周期，只有稍后finalizer会调用close。教程自己的大多数示例显式close恰好说明确定性资源管理仍由应用负责，与同页文字冲突。
- 根因：文档把“callback异常由wrapper立即关闭”与“不可达对象最终由GC关闭”合并成“callback返回自动关闭”，混淆控制流结束和非确定性finalization；runtime没有为成功callback提供scope guard/defer或明确handoff协议。
...
静态封板统计为376项：P0为0，P1为114，P2为204，P3为58。模块分布：CORE 13、METRIC 11、NET 8、SOCK 20、UDP 1、TLS 18、DNS 18、CLUSTER 19、ADDR 2、URL 3、HTTPC 9、HTTP1 23、COMP 1、WS 11、H2 41、HPACK 3、GRPC 39、REDIS 10、MYSQLC 9、MYSQL 20、ETCD 17、DOC 80。逐文件台账覆盖389个唯一实际文件且双向差集为空，没有`待审`或`审阅中`项；这只是定义范围内的静态审阅封板数，不是“实现没有bug”的证明。

静态封板不等于代码可发布：114项P1默认全部阻断1.0；204项P2中涉及wire framing/状态机、数据或事务一致性、认证、无限等待、资源泄漏、close后复活及跨平台未定义行为的条目也按阻断处理，除非维护者逐项书面接受风险并给出部署缓解。按用户要求延期的独立peer、畸形输入、并发barrier、fault injection、版本矩阵和修复后sanitizer保留到修复阶段；动态结果或后续代码变更仍可能发现新问题。
...
- 2026-08-13：完成最终静态封板核账：`src/**`、`luaclib-src/**`、`lualib/**`、`test/**`、双语Markdown与VuePress配置共389个实际文件，与逐文件台账双向差集为空；250项已审有归档、80项已审无新增、59项范围外，没有待审或审阅中。主报告与HANDOFF最终一致为376项master问题及4项cluster分支独有问题，1.0继续阻断。
