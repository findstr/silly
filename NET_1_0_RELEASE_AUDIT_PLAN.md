...
- HTTP监控依赖：`lualib/silly/metrics/` runtime、默认collectors、`testprometheus.lua`及Collector/Counter/Gauge/Histogram/Labels/Prometheus/Registry七组双语reference已逐段收口；高基数raw path在最佳实践、logging guide、Counter/Histogram和Prometheus reference的重复位置统一归入`DOC-051`，异步operation错误使用CPU clock见`DOC-054`，logger formatter契约见`DOC-057`，其余wire/schema/生命周期问题由`METRIC-001`至`011`覆盖。
