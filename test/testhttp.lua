...
testaux.case("Test 26: read() timeout and retry - Content-Length", function()
	local ch1 = channel.new()  -- client -> server
	local ch2 = channel.new()  -- server -> client
	server_handler = function(stream)
		stream:respond(200, {
			["content-type"] = "text/plain",
			["content-length"] = 20,
		...
		stream:write("1")
		stream:flushwrite()
		ch1:pop()  -- wait for client signal
		-- Write partial data, then delay
		stream:write("2345")
		stream:flushwrite()
		ch2:push("")  -- notify client
		stream:write("67890")
		ch1:pop()  -- wait for client signal
	...
	-- Read with short timeout - may timeout on first read
	local chunk1, err = stream:read(5, 10)
	...
	testaux.asserteq(stream.err, nil, "Test 26.4: stream.err should not be set on timeout")
	ch1:push("")  -- notify server

	ch2:pop()  -- wait for server signal
	local chunk2, err = stream:read(5, 10000)
	testaux.asserteq(chunk2, "12345", "Test 26.5: chunk2 should be 12345")
	testaux.asserteq(err, nil, "Test 26.6: err should be nil")
	testaux.asserteq(stream.eof, false, "Test 26.7: EOF should be false")
	-- Read rest
	local rest, err = stream:readall(100)
	testaux.asserteq(rest, nil, "Test 26.8: rest should be nil on timeout")
	testaux.asserteq(err, "read timeout", "Test 26.9: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 26.10: stream.err should not be set on timeout")
	ch1:push("")  -- notify server

	rest, err = stream:readall(100)
	testaux.asserteq(rest, "67890ABCDEFGHIJ", "Test 26.11: Rest should be complete")
	testaux.asserteq(err, nil, "Test 26.12: err should be nil")
	testaux.asserteq(stream.eof, true, "Test 26.13: EOF should be true")
	testaux.asserteq(stream.err, nil, "Test 26.14: stream.err should not be set on timeout")

	wait_done()
end)

testaux.case("Test 27: read() timeout and retry - Chunked", function()
	local ch1 = channel.new()  -- client -> server
	local ch2 = channel.new()  -- server -> client
	server_handler = function(stream)
		stream:respond(200, {
	...
	local chunk1, err = stream:read(5, 10)
	testaux.asserteq(chunk1, nil, "Test 27.2: chunk1 should be nil on timeout")
	testaux.asserteq(err, "read timeout", "Test 27.3: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 27.4: stream.err should not be set on timeout")
	ch1:push("")  -- notify server

	-- Retry should succeed
	ch2:pop()  -- wait for server signal
	chunk1, err = stream:read(5, 1000)
	testaux.asserteq(chunk1, "AAAAA", "Test 27.5: chunk1 should be AAAAA")
	testaux.asserteq(err, nil, "Test 27.6: err should be nil")
	testaux.asserteq(stream.eof, false, "Test 27.7: EOF should be false")

	-- readall with short timeout - should timeout again
	local rest, err = stream:readall(20)
	testaux.asserteq(rest, nil, "Test 27.8: rest should be nil on timeout")
	testaux.asserteq(err, "read timeout", "Test 27.9: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 27.10: stream.err should not be set on timeout")
	ch1:push("")  -- notify server

	-- readall retry should succeed
	rest, err = stream:readall(1000)
	testaux.asserteq(rest, "BBBBBCCCCC", "Test 27.11: rest should be BBBBBCCCCC")
	testaux.asserteq(err, nil, "Test 27.12: err should be nil")
	testaux.asserteq(stream.eof, true, "Test 27.13: EOF should be true")

	local total = chunk1 .. rest
	testaux.asserteq(total, "AAAAABBBBBCCCCC", "Test 27.14: Total should be complete")

	wait_done()
end)

testaux.case("Test 28: readall() timeout and retry", function()
	local ch1 = channel.new()  -- client -> server
	local ch2 = channel.new()  -- server -> client
	server_handler = function(stream)
		stream:respond(200, {
	...
	-- readall with short timeout - should timeout
	local body, err = stream:readall(10)
	testaux.asserteq(body, nil, "Test 28.2: body should be nil on timeout")
	testaux.asserteq(err, "read timeout", "Test 28.3: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 28.4: stream.err should not be set")
	ch1:push("")  -- notify server

	-- readall with short timeout again - should timeout again
	ch2:pop()  -- wait for server signal
	body, err = stream:readall(10)
	testaux.asserteq(body, nil, "Test 28.5: body should be nil on second timeout")
	testaux.asserteq(err, "read timeout", "Test 28.6: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 28.7: stream.err should not be set")
	ch1:push("")  -- notify server

	-- Retry with longer timeout - should succeed
	body, err = stream:readall(1000)
	testaux.asserteq(body, "1234567890ABCDEFGHIJ1234567890", "Test 28.8: Should get full body")
	testaux.asserteq(err, nil, "Test 28.9: err should be nil")
	testaux.asserteq(stream.eof, true, "Test 28.10: EOF should be true")

	wait_done()
end)

testaux.case("Test 29: read() exceeding Content-Length", function()
	server_handler = function(stream)
		stream:respond(200, {
			["content-length"] = 10,
		})
	...
	testaux.asserteq(chunk1, "1234567890", "Test 29.2: Read exact length")
	testaux.asserteq(stream.eof, true, "Test 29.3: EOF should be true")

	-- Try to read more
	local chunk2, err = stream:read(10)
	testaux.asserteq(chunk2, "", "Test 29.4: Read after EOF should return empty")
	testaux.asserteq(err, "end of file", "Test 29.5: Should get end of file error")

	wait_done()
end)

testaux.case("Test 30: read() after chunked ends", function()
	server_handler = function(stream)
		stream:respond(200, {
			["transfer-encoding"] = "chunked",
		})
	...
	-- Read all data
	local body, err = stream:readall()
	testaux.asserteq(body, "AAAAABBBBB", "Test 30.2: Should read all chunks")
	testaux.asserteq(stream.eof, true, "Test 30.3: EOF should be true")

	-- Try to read more
	local chunk, err = stream:read(10)
	testaux.asserteq(chunk, "", "Test 30.4: Read after EOF should return empty")
	testaux.asserteq(err, "end of file", "Test 30.5: Should get end of file error")

	wait_done()
end)

testaux.case("Test 31: read() partial, then readall() rest - Content-Length", function()
	server_handler = function(stream)
		stream:respond(200, {
			["content-length"] = 20,
		})
	...
	-- Read partial
	local chunk1, err = stream:read(5)
	testaux.asserteq(chunk1, "12345", "Test 31.2: First read should get 5 bytes")
	testaux.asserteq(stream.eof, false, "Test 31.3: EOF should be false")

	-- readall() should get the rest
	local rest, err = stream:readall()
	testaux.asserteq(rest, "678901234567890", "Test 31.4: readall should get remaining 15 bytes")
	testaux.asserteq(stream.eof, true, "Test 31.5: EOF should be true")

	wait_done()
end)

testaux.case("Test 32: read() partial, then readall() rest - Chunked", function()
	server_handler = function(stream)
		stream:respond(200, {
			["transfer-encoding"] = "chunked",
		})
	...
	local chunk1, err = stream:read(7)
	testaux.asserteq(chunk1, "AAAAABB", "Test 32.2: First read should span chunks")

	-- readall() should get the rest
	local rest, err = stream:readall()
	testaux.asserteq(rest, "BBBCCCCCDDDDD", "Test 32.3: readall should get rest")
	testaux.asserteq(stream.eof, true, "Test 32.4: EOF should be true")

	local total = chunk1 .. rest
	testaux.asserteq(total, "AAAAABBBBBCCCCCDDDDD", "Test 32.5: Total should be complete")

	wait_done()
end)

testaux.case("Test 33: Connection broken - Content-Length incomplete", function()
	server_handler = function(stream)
		stream:respond(200, {
			["content-length"] = 100,
		})
	...
	local body, err = stream:readall()
	testaux.asserteq(body, nil, "Test 33.2: readall should fail")
	testaux.assertneq(err, nil, "Test 33.3: Should have error")
	testaux.assertneq(stream.err, nil, "Test 33.4: stream.err should be set (connection dead)")

	-- Try to read again - should fail immediately
	local chunk, err2 = stream:read(10)
	testaux.asserteq(chunk, nil, "Test 33.5: read should fail")
	testaux.asserteq(err2, stream.err, "Test 33.6: Should return cached error")
end)

testaux.case("Test 34: Connection broken - Chunked incomplete", function()
	server_handler = function(stream)
		stream:respond(200, {
			["transfer-encoding"] = "chunked",
		})
	...
	local stream<close>, err = httpc:request("GET", "http://127.0.0.1:8080", {})
	testaux.assertneq(stream, nil, "Test 34.1: Request should succeed")
	stream:waitresponse()
	...
	local body, err = stream:readall()
	testaux.asserteq(body, nil, "Test 34.2: readall should fail")
	testaux.assertneq(err, nil, "Test 34.3: Should have error")
	testaux.assertneq(stream.err, nil, "Test 34.4: stream.err should be set")
end)

testaux.case("Test 35: Mixed read/readall with multiple timeouts", function()
	local ch1 = channel.new()  -- client -> server
	local ch2 = channel.new()  -- server -> client
	server_handler = function(stream)
		stream:respond(200, {
		...
		-- Send data in chunks with synchronization
		stream:write("AAAAA")
		stream:flushwrite()
		ch1:pop()  -- wait for client signal
		stream:write("BBBBB")
		...
		stream:write("CCCCC")
		ch1:pop()  -- wait for client signal
		local ok, err = stream:write("DDDDDEEEEEFFFFFFX")
		testaux.asserteq(ok, false, "Test 35.1: write large than content-length size should fail")
		testaux.asserteq(err, "write exceed content-length size", "Test 35.2: err should be write exceed content-length size")
		local ok, err = stream:write("DDDDDEEEEEFFFFFF")
		testaux.asserteq(ok, true, "Test 35.1: write should succeed")
		testaux.asserteq(err, nil, "Test 35.1: err should be nil")
	end

	local stream<close>, err = httpc:request("GET", "http://127.0.0.1:8080", {})
	testaux.assertneq(stream, nil, "Test 35.1: Request should succeed")
	stream:waitresponse()

	-- Read with short timeout - should timeout
	local chunk1, err = stream:read(10, 10)
	testaux.asserteq(chunk1, nil, "Test 35.2: chunk1 should be nil on timeout")
	testaux.asserteq(err, "read timeout", "Test 35.3: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 35.4: stream.err should not be set on timeout")
	ch1:push("")  -- notify server

	-- Retry should succeed
	ch2:pop()  -- wait for server signal
	chunk1, err = stream:read(10, 1000)
	testaux.asserteq(chunk1, "AAAAABBBBB", "Test 35.5: First read should succeed after retry")
	testaux.asserteq(err, nil, "Test 35.6: err should be nil")
	testaux.asserteq(#chunk1, 10, "Test 35.7: Should get 10 bytes")
	testaux.asserteq(stream.eof, false, "Test 35.8: EOF should be false")

	-- readall with short timeout - should timeout again
	local rest, err = stream:readall(10)
	testaux.asserteq(rest, nil, "Test 35.9: rest should be nil on timeout")
	testaux.asserteq(err, "read timeout", "Test 35.10: err should be read timeout")
	testaux.asserteq(stream.err, nil, "Test 35.11: stream.err should not be set on timeout")
	ch1:push("")  -- notify server

	-- readall retry should succeed
	rest, err = stream:readall(1000)
	testaux.asserteq(rest, "CCCCCDDDDDEEEEEFFFFFF", "Test 35.12: readall should succeed")
	testaux.asserteq(err, nil, "Test 35.13: err should be nil")
	testaux.asserteq(stream.eof, true, "Test 35.14: EOF should be true")

	local total = chunk1 .. rest
	testaux.asserteq(total, "AAAAABBBBBCCCCCDDDDDEEEEEFFFFFF", "Test 35.15: Total should be complete")

	wait_done()
end)

testaux.case("Test 36: read() returns buffered data before network read", function()
	server_handler = function(stream)
		stream:respond(200, {
			["content-length"] = 20,
		})
	...
	local stream<close>, err = httpc:request("GET", "http://127.0.0.1:8080", {})
	testaux.assertneq(stream, nil, "Test 36.1: Request should succeed")
	stream:waitresponse()

	-- Read more than available in one request
	local chunk1, err = stream:read(30)
	testaux.asserteq(chunk1, "", "Test 36.2: Should read only available 20 bytes")
	testaux.asserteq(stream.eof, true, "Test 36.3: EOF should be true")

	-- Buffer should be empty now
	local chunk2, err = stream:read(20)
	testaux.asserteq(chunk2, "12345678901234567890", "Test 36.4: Should read all 20 bytes")
	testaux.asserteq(stream.eof, true, "Test 36.5: EOF should be true")

	wait_done()
end)

testaux.case("Test 37: GET with gzip compression", function()
	...
	server_handler = function(stream)
		...
		testaux.assertneq(compressed, nil, "Test 37.1: gzip.compress should succeed")
		testaux.asserteq(err, nil, "Test 37.2: gzip.compress should not return error")
	...
	local response = httpc:get("http://127.0.0.1:8080")
	testaux.assertneq(response, nil, "Test 37.3: GET should succeed")
	testaux.asserteq(response.status, 200, "Test 37.4: Status should be 200")
	testaux.asserteq(response.body, original_data, "Test 37.5: Body should be automatically decompressed")
	testaux.asserteq(response.header["content-encoding"], "gzip", "Test 37.6: Content-Encoding header should be preserved")
	wait_done()
end)

testaux.case("Test 38: POST with gzip compression response", function()
	...
	server_handler = function(stream)
		local body, err = stream:readall()
		testaux.asserteq(body, request_data, "Test 38.1: Server should receive uncompressed request")

		-- Compress the response
		local compressed, err = gzip.compress(response_data)
		testaux.assertneq(compressed, nil, "Test 38.2: gzip.compress should succeed")
	...
	testaux.assertneq(response, nil, "Test 38.3: POST should succeed")
	testaux.asserteq(response.status, 200, "Test 38.4: Status should be 200")
	testaux.asserteq(response.body, response_data, "Test 38.5: Body should be automatically decompressed")
	wait_done()
end)

testaux.case("Test 39: GET without gzip (no Content-Encoding)", function()
	...
	local response = httpc:get("http://127.0.0.1:8080")
	testaux.assertneq(response, nil, "Test 39.1: GET should succeed")
	testaux.asserteq(response.body, original_data, "Test 39.2: Body should remain uncompressed")
	wait_done()
end)

testaux.case("Test 40: GET with Accept-Encoding header check", function()
	server_handler = function(stream)
		local accept_encoding = stream.header["accept-encoding"]
		testaux.asserteq(accept_encoding, "gzip", "Test 40.1: Client should send Accept-Encoding: gzip")
	...
	local response = httpc:get("http://127.0.0.1:8080")
	testaux.asserteq(response.body, "OK", "Test 40.2: Response should be OK")
	wait_done()
end)
