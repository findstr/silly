...
testaux.case("Test 20: Connection broken during read", function()
	...
	testaux.asserteq(err, "channel closed", "Test 20.3: Should have error")
...
testaux.case("Test 25: HTTP/2 cocurrent stream stream", function()
	...
	local s = ch:openstream()
	testaux.asserteq(block, true, "Test 24.5: Openstream should be block")
	testaux.assertneq(s, nil, "Test 24.5: Openstream should be success")

	local wg = waitgroup.new()
	for i = 1, 100 do
		wg:fork(function()
			local s, err = ch:openstream()
			testaux.asserteq(s, nil, "Test 25.6: Openstream should be ok")
			testaux.asserteq(err, "channel closed", "Test 25.6: Openstream should be ok")
		end)
	end
	wg:fork(function()
		ch:close()
...
testaux.case("Test 26: HTTP/2 opensream on a closed channel", function()
	...
	ch:close()
	local ok, err = s:request("GET", "/", {})
	testaux.asserteq(ok, false, "Test 26.4: Request should fail")
	testaux.asserteq(err, "channel closed", "Test 26.4: Request should fail")
	s:closewrite("foo")
	local body, err = s:readall()
	...
	testaux.asserteq(err, "channel closed", "Test 26.5: Openstream should fail")
end)
...
testaux.case("Test 27: HTTP/2 streamcount accuracy with concurrent open/close", function()
	...
	ch:close()
	time.sleep(100)
	testaux.asserteq(ch.streamcount, 0, "Test 27.17: streamcount should be 0 after channel close")
end)
