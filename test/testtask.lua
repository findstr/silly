...
testaux.case("Test 2: lifecycle", function()
	...
	local co2 = silly.fork(function()end)
	testaux.asserteq(silly.status(co2), "READY", "Test 2.7: initial status is READY")
...
testaux.case("Test 3: wait and wakeup", function()
	local co
	local ok = false
	co = silly.fork(function()
		testaux.asserteq(silly.status(co), "RUN", "Test 3.1: run status is RUN")
		local value = silly.wait()
		testaux.asserteq(silly.status(co), "RUN", "Test 3.2: run status is RUN")
		testaux.asserteq(value, "test_value", "Test 3.3: wait hasn't returned")
		ok = true
	end)
	testaux.asserteq(silly.status(co), "READY", "Test 3.4: fork status is READY")
	time.sleep(0)
	testaux.asserteq(silly.status(co), "WAIT", "Test 3.5: wait status is WAIT")
	silly.wakeup(co, "test_value")
	while not ok do
		time.sleep(100)
	end
	testaux.asserteq(silly.status(co), nil, "Test 3.6: status cleaned after completion")
end)
...
testaux.case("Test 4: error handling", function()
	...
	testaux.asserteq(silly.status(co), "READY", "Test 4.1: status before execution")
...
testaux.case("Test 5: multiple wait/wakeup", function()
	...
	local co
	co = silly.fork(function()
		results[1] = silly.wait()
		results[2] = silly.wait()
		results[3] = silly.wait()
	end)

	time.sleep(0)  -- Enter first wait
	testaux.asserteq(silly.status(co), "WAIT", "Test 5.1: first wait")
	silly.wakeup(co, "a")

	time.sleep(0)  -- Enter second wait
	testaux.asserteq(silly.status(co), "WAIT", "Test 5.2: second wait")
	silly.wakeup(co, "b")

	time.sleep(0)  -- Enter third wait
	testaux.asserteq(silly.status(co), "WAIT", "Test 5.3: third wait")
	silly.wakeup(co, "c")
...
testaux.case("Test 6: invalid operations", function()
	...
	silly.wakeup(co2, nil)
	-- Wakeup coroutine in RUN state
	local co3
	co3 = silly.fork(function()
		ok, err = pcall(silly.wakeup, co2, nil)
...
testaux.case("Test 8: orphaned WAIT tasks (strong reference)", function()
	...
	for i = 1, 5 do
		orphans[i] = silly.fork(function()
			silly.wait()  -- Wait forever
		end)
	end
	...
	for i = 1, 5 do
		silly.wakeup(orphans[i], "cleanup")
	end
