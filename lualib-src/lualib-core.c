...
typedef int64_t(connect_t)(const char *ip, const char *port, const char *bip,
		       const char *bport);
...
static int ltcplisten(lua_State *L)
{
	...
	int64_t fd = silly_socket_listen(ip, port, backlog);
	if (unlikely(fd < 0)) {
		lua_pushnil(L);
		lua_pushstring(L, silly_socket_lasterror());
	} else {
...
static int ludpbind(lua_State *L)
{
	const char *ip = luaL_checkstring(L, 1);
	const char *port = luaL_checkstring(L, 2);
	int64_t fd = silly_socket_udpbind(ip, port);
	if (unlikely(fd < 0)) {
		lua_pushnil(L);
		lua_pushstring(L, silly_socket_lasterror());
	} else {
